    ...
    window.Game = {
        ...
        handleCorrectAnswer() {
            if (this.isRecovering) {
                ...
                // Pressure: enviar ataque al objetivo restando tiempo
                if (window.RuneState && RuneState.pressureActive && RuneState.pressureTarget && typeof Multiplayer !== 'undefined') {
                    if (RuneState.pressureTarget !== Users.current) {
                        Multiplayer.sendAttack(Users.current, RuneState.pressureTarget, 'rune_time_curse');
                    }
                }
        ...
        showFocusWordsFeedback() {
            ...
            const sorted = Object.keys(this.wordColors)
                .filter(w => this.wordColors[w].color === 'rojo')
                .slice(0, 3)
                .map(w => [w, this.wordColors[w].color]);
            ...
            sorted.forEach(([word, color]) => {
                ...
                const colorHex = color === 'rojo' ? '#ff4444' : '#f2c94c';
                html += `
                    <div style="display:flex; justify-content:space-between; align-items:center; 
                        ...
                        <span style="color:${colorHex}; font-size:0.6em;">●</span>
                    </div>`;
            });
            html += '</div>';
